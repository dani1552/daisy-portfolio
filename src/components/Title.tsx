...
const TypingTextDemo = ({ delay, holdDelay, loop, cursor }: TypingTextDemoProps) => {
  return (
    <TypingText
      ...
      delay={delay}
      holdDelay={holdDelay}
      className="whitespace-pre-line break-keep text-[clamp(1.2rem,3.8vw,2.3rem)] leading-[clamp(2rem,4vw,8rem)] font-semibold text-white sm:text-[clamp(1.4rem,3vw,2.7rem)] sm:leading-[clamp(2rem,3.2vw,2.6rem)] md:text-[clamp(1.8rem,2.3vw,3.25rem)] md:leading-[clamp(2.6rem,3vw,4.5rem)]"
...
export default function Title() {
  return (
    <section
      className="relative flex flex-col items-center justify-center bg-transparent px-6 py-16"
      style={{
        height: "calc(100dvh - var(--header-height, 0px))",
        minHeight: "calc(100dvh - var(--header-height, 0px))",
      }}
    >
      <div className="max-w-6xl mx-auto text-left flex-1 flex items-center">
